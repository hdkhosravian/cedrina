...
from tests.utils.security_helpers import SecurityTestHelpers, SecurityAssertions
...
class TestLogoutEndpointSecurityIntegration:
    ...
    def test_logout_rejects_cross_user_tokens(self, mock_get_current_user, mock_get_token_service, test_client):
        """
...
class TestSecurityPatternCompliance:
    ...
    async def test_token_ownership_validation_pattern_compliance(self):
        ...
        with pytest.raises(Exception):  # Should raise AuthenticationError
            await SecurityTestHelpers.validate_token_ownership_pattern(
                other_token,
                self.test_user
            )

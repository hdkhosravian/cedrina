def test_access_during_emergency_override(client, regular_user_headers, admin_user_headers):
    ...
    for policy in policies_data["policies"]:
        if (
            policy["subject"] == "emergency_team"
            and policy["object"] == "/api/v1/emergency-resource"
            and policy["action"] == "GET"
        ):
    ...
    response = client.post(
        "/api/v1/admin/policies/remove", json=removal_data, headers=admin_headers
    )
    assert response.status_code == 200, f"Expected 200, got {response.status_code}"
    ...
    assert response_data["action"] == "GET"
    assert (
        "Policy removed successfully" in response_data["message"]
        or "Policy not found" in response_data["message"]
    )

...
def test_audit_logging_for_policy_changes(client, admin_user_headers, caplog):
    ...
    added_records = [record for record in caplog.records if 'Policy added:' in record.message and 'audit_test_user' in record.message]
    exists_records = [record for record in caplog.records if 'Policy already exists:' in record.message and 'audit_test_user' in record.message]
    audit_records = [record for record in caplog.records if 'Audit log recorded for' in record.message and 'audit_test_user' in record.message]
    ...
    assert len(added_records) >= 1 or len(exists_records) >= 1 or len(audit_records) >= 1, f'Expected at least one log entry for policy operation. Found {len(caplog.records)} total records'
    
    # Check for audit log entries (when a policy is actually added)
    audit_records = [record for record in caplog.records if 'Audit log recorded for add' in record.message]
    ...
    not_found_records = [record for record in caplog.records if 'Policy not found:' in record.message]
    
    # Either the policy was successfully removed or it wasn't found (both are valid audit events)
    assert len(removal_records) >= 1 or len(not_found_records) >= 1, 'Expected at least one log entry for policy removal attempt'

def test_abac_with_time_based_access(client, regular_user_headers, admin_user_headers):
    ...
    for policy in policies_data["policies"]:
        if (
            policy["subject"] == "user"
            and policy["object"] == "/api/v1/test-resource"
            and policy["action"] == "GET"
            and policy.get("attributes", {}).get("time_of_day") == "working_hours"
        ):
            added_policy = policy
            break

    assert (
        added_policy is not None
    ), "ABAC policy with time_of_day attribute was not found in policy list"

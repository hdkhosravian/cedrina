...
@pytest.mark.asyncio
async def test_user_access_based_on_department_and_location(client: TestClient, regular_user: User, regular_user_headers: dict):
    """
    ...
    response = client.post('/api/v1/admin/policies/add', json=policy_data_restrictive, headers=headers)
    assert response.status_code == 200, f'Expected 200, got {response.status_code}' 
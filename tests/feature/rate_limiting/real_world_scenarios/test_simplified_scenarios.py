...
class TestSimplifiedRateLimitingScenarios:
    ...
    async def test_new_user_registration_flow(
        self, scenario_state: ScenarioState, user_scenarios: dict
    ):
        ...
        with patch("src.core.rate_limiting.services.AdvancedRateLimiter") as mock_limiter_class:
            ...
            for i in range(4):
                rate_limit_result = await mock_limiter.check_rate_limit(
                    key="registration:192.168.1.1", context={"endpoint": "/api/v1/auth/register"}
                )
            ...
            print("   ✅ Rate limiting activated after 3 attempts")
        ...
        print(
            f"✅ Registration flow completed: {summary['total_requests']} requests, {summary['rate_limit_hits']} rate limited"
        )

    @pytest.mark.asyncio
    async def test_api_usage_patterns(self, scenario_state: ScenarioState, user_scenarios: dict):
    ...
    async def test_tier_based_rate_limiting(
        ...
        for user, tier in test_users:
            ...
            with patch(
                ...
                assert (
                    successful == tier_limit
                ), f"{tier} tier should allow exactly {tier_limit} requests"
                assert failed == 5, f"{tier} tier should deny 5 excess requests"

                print(
                    f"     ✅ {tier.title()} tier: {successful}/{test_requests} requests allowed (limit: {tier_limit})"
                )
        ...
        print(
            f"✅ Tier-based limiting tested: {tier_summary['total_requests']} requests across all tiers"
        )

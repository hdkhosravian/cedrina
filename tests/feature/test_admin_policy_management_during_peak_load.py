...
def test_admin_policy_management_during_peak_load(client, admin_user_headers):
    ...
    assert success_count + already_exists_count == 5, "Not all policy operations completed successfully"
    ...
    assert response.status_code == 200, f"Redis watcher test policy addition failed with {response.status_code}"

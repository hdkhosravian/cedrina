...
@pytest.mark.asyncio
async def test_rate_limit_enforcement_during_policy_update(
    client: TestClient, admin_user: User, admin_user_headers: dict
):
    ...
    for policy in policy_list["policies"]:
        if (
            policy.get("subject") == "rate_test_user"
            and policy.get("object") == "/rate-limit-test"
            and policy.get("action") == "GET"
        ):
            found_policy = True
            break

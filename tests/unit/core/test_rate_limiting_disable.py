...
from src.domain.rate_limiting.entities import RateLimitRequest, RateLimitResult
from src.domain.rate_limiting.value_objects import RateLimitKey, RateLimitAlgorithm
...
class TestRateLimitingConfigDisable:
    ...
    def test_global_disable_via_disable_rate_limiting(self):
        ...
        assert config.get_bypass_reason() == "Rate limiting globally disabled via RATE_LIMITING_DISABLED"
    ...
    def test_global_disable_via_emergency_disable(self):
        ...
        assert config.is_rate_limiting_disabled() is True
        assert config.should_bypass_rate_limit() is True
        assert config.get_bypass_reason() == "Rate limiting disabled via emergency override (RATE_LIMITING_EMERGENCY_DISABLE)"
    ...
    def test_global_disable_via_enable_rate_limiting_false(self):
        ...
        assert config.is_rate_limiting_disabled() is True
        assert config.should_bypass_rate_limit() is True
        assert config.get_bypass_reason() == "Rate limiting globally disabled via RATE_LIMITING_ENABLED=false"
    ...
    def test_ip_based_bypass(self):
        ...
        assert config.get_bypass_reason(client_ip="192.168.1.1") == "Rate limiting disabled for IP: 192.168.1.1"
    ...
    def test_endpoint_based_bypass(self):
        ...
        assert config.get_bypass_reason(endpoint="/api/v1/health") == "Rate limiting disabled for endpoint: /api/v1/health"
    ...
    def test_tier_based_bypass(self):
        ...
        assert config.get_bypass_reason(user_tier="premium") == "Rate limiting disabled for tier: premium"
    ...
    def test_multiple_bypass_conditions(self):
        ...
        assert config.should_bypass_rate_limit(

...
def mock_enforce(*args, **kwargs):
    ...
    if subject != "admin" or not subject:
        if resource in protected_endpoints:
            print(
                f"Mock enforce: Denying access to {resource} for non-admin or empty subject {subject}"
            )
            return False
...
def test_enforcer_policy_enforcement_admin_access(get_enforcer):
    ...
    assert enforcer.enforce(
        "admin", "/health", "GET", "*", "*", "*"
    ), "Admin should have access to /health"
    assert enforcer.enforce(
        "admin", "/metrics", "GET", "*", "*", "*"
    ), "Admin should have access to /metrics"
    assert enforcer.enforce(
        "admin", "/docs", "GET", "*", "*", "*"
    ), "Admin should have access to /docs"
    assert enforcer.enforce(
        "admin", "/redoc", "GET", "*", "*", "*"
    ), "Admin should have access to /redoc"
...
def test_enforcer_policy_enforcement_non_admin_denied(get_enforcer):
    ...
    assert not enforcer.enforce(
        test_subject, "/health", "GET", "*", "*", "*"
    ), f"Non-admin {test_subject} should not have access to /health"
    assert not enforcer.enforce(
        test_subject, "/metrics", "GET", "*", "*", "*"
    ), f"Non-admin {test_subject} should not have access to /metrics"
    assert not enforcer.enforce(
        test_subject, "/docs", "GET", "*", "*", "*"
    ), f"Non-admin {test_subject} should not have access to /docs"
    assert not enforcer.enforce(
        test_subject, "/redoc", "GET", "*", "*", "*"
    ), f"Non-admin {test_subject} should not have access to /redoc"
...
def test_enforcer_invalid_subject(get_enforcer):
    ...
    assert not enforcer.enforce(
        "", "/health", "GET", "*", "*", "*"
    ), "Empty subject should not have access to /health"
    assert not enforcer.enforce(
        "", "/metrics", "GET", "*", "*", "*"
    ), "Empty subject should not have access to /metrics"
    assert not enforcer.enforce(
        "", "/docs", "GET", "*", "*", "*"
    ), "Empty subject should not have access to /docs"
    assert not enforcer.enforce(
        "", "/redoc", "GET", "*", "*", "*"
    ), "Empty subject should not have access to /redoc"

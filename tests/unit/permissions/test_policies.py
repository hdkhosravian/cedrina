...
def mock_enforce(*args, **kwargs):
    ...
    if policy_key in _simulated_policies:
        print(
            f"Mock enforce: Allowing access to {resource} for subject {subject} based on policy store"
        )
        return True
...
def add_policy(subject, resource, action, dept="*", loc="*", time="*"):
    ...
    print(
        f"Policy added: {subject} can {action} on {resource} with dept={dept}, loc={loc}, time={time}"
    )
    return True
...
def remove_policy(subject, resource, action, dept="*", loc="*", time="*"):
    ...
    print(
        f"Policy removed: {subject} can no longer {action} on {resource} with dept={dept}, loc={loc}, time={time}"
    )
    return True
...
def test_add_and_remove_policy_cycle(get_enforcer):
    ...
    ), "Access should be granted after policy addition"
    ...
    assert not enforcer.enforce(
        subject, resource, action, "*", "*", "*"
    ), "Access should be denied after policy removal"

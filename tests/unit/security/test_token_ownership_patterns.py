...
class TestTokenOwnershipValidationPattern:
    ...
    async def validate_token_ownership(self, token: str, current_user: User, language: str = "en") -> dict:
        """
    ...
    async def test_expired_token_rejected(self, primary_user):
        ...
        expired_token = jwt.encode(payload, settings.JWT_PRIVATE_KEY.get_secret_value(), algorithm="RS256")
    ...
    async def test_invalid_issuer_rejected(self, primary_user):
        ...
        wrong_issuer_token = jwt.encode(payload, settings.JWT_PRIVATE_KEY.get_secret_value(), algorithm="RS256")
    ...
    async def test_invalid_audience_rejected(self, primary_user):
        ...
        wrong_audience_token = jwt.encode(payload, settings.JWT_PRIVATE_KEY.get_secret_value(), algorithm="RS256")
...
class TestSecureEndpointPatterns:
    ...
    @pytest.fixture
    def user_one(self):
        return User(id=1, username="user_one", email="one@example.com", role=Role.USER, is_active=True)

    @pytest.fixture
    def user_two(self):
        return User(id=2, username="user_two", email="two@example.com", role=Role.USER, is_active=True)
    ...
    async def vulnerable_refresh_endpoint(self, refresh_token: str, current_user: User, token_service):
        """
    ...
    @pytest.mark.asyncio
    async def test_secure_pattern_prevents_cross_user_attack(self, user_one, user_two, mock_token_service):
        """
    ...
    async def test_vulnerable_pattern_demonstration(self, user_one, user_two, mock_token_service):
        ...
        result = await self.vulnerable_refresh_endpoint(user_two_token, user_one, mock_token_service)
...
class TestExistingServiceSecurity:
    ...
    @pytest.mark.asyncio
    async def test_token_service_refresh_tokens_validates_ownership(self, mock_db_session, mock_redis_client, mock_session_service):
        """
        ...
        # Create test users
        user_one = User(id=1, username="user_one", email="one@example.com", role=Role.USER, is_active=True)
        user_two = User(id=2, username="user_two", email="two@example.com", role=Role.USER, is_active=True)
        ...
        user_two_token = jwt.encode({
            ...
            "iss": settings.JWT_ISSUER,
            "aud": settings.JWT_AUDIENCE
        }, settings.JWT_PRIVATE_KEY.get_secret_value(), algorithm="RS256")
    ...
    @pytest.mark.asyncio 
    async def test_session_service_revoke_token_validates_ownership(self, mock_db_session, mock_redis_client):
        """
        ...
        # Create test user
        user = User(id=1, username="test_user", email="test@example.com", role=Role.USER, is_active=True)

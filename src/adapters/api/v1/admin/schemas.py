...
class PolicyRequest(BaseModel):
    ...
    """
    subject: str = Field(..., description="The subject (e.g., role) for the policy", min_length=1, max_length=255)
    object: str = Field(..., description="The resource/object for the policy", min_length=1, max_length=255)
    action: str = Field(..., description="The action for the policy", min_length=1, max_length=255)
    sub_dept: Optional[str] = Field(None, description="Department attribute for ABAC", max_length=255)
    sub_loc: Optional[str] = Field(None, description="Location attribute for ABAC", max_length=255)
    time_of_day: Optional[str] = Field(None, description="Time of day attribute for ABAC", max_length=255)

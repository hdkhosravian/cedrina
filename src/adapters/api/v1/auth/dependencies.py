...
def get_user_auth_service(db: AsyncDB) -> UserAuthenticationService:
    """Factory that returns :class:`UserAuthenticationService`."""
    return UserAuthenticationService(db)


def get_token_service(db: AsyncDB, redis: RedisClient) -> TokenService:
    """Factory that returns :class:`TokenService`."""
    return TokenService(db, redis)


def get_oauth_service(db: AsyncDB) -> OAuthService:

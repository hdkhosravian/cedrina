...
class UserRepository(IUserRepository):
    ...
    def __init__(self, db_session: AsyncSession):
        ...
        logger.debug(
            ...
            responsibilities=["user_persistence", "value_object_integration", "transaction_management"]
        )
        ...
        try:
            if user.id is None:
                ...
                logger.debug(
                    ...
                    email=user.email[:3] + "***" if user.email and len(user.email) > 3 else user.email,
            ...
            logger.error(
                ...
                username=user.username[:3] + "***" if user and user.username and len(user.username) > 3 else (user.username if user else None),
                error=str(e),
                error_type=type(e).__name__,

...
from .value_objects import RateLimitKey, RateLimitQuota, RateLimitAlgorithm, RateLimitWindow
    ...
    def matches_request_context(
        ...
        if self.endpoints and endpoint:
            endpoint_matches = any(
                self._endpoint_matches(endpoint, pattern)
        ...
        if self.ip_ranges and client_ip:
            ip_matches = any(
                self._ip_matches(client_ip, ip_range)
...
class RateLimitRequest:
    ...
    def is_high_priority(self) -> bool:
        ...
        return (
            self.user_tier in high_priority_tiers or
            self.endpoint in high_priority_endpoints or
            self.get_custom_attribute("high_priority", False)
        )

...
class RateLimitRepository(ABC):
    ...
    @abstractmethod
    async def update_token_bucket_state(
        self, key: "RateLimitKey", last_refill: float, tokens: float
    ) -> None:
    ...
    @abstractmethod
    async def get_sliding_window_requests(
        self, key: "RateLimitKey", window_start: float
    ) -> List[float]:
    ...
    @abstractmethod
    async def increment_fixed_window_count(
        self, key: "RateLimitKey", window_start: float, timestamp: float
    ) -> int:
...
class RateLimitMetricsRepository(ABC):
    ...
    async def get_metrics_summary(
        self, start_time: datetime, end_time: datetime, granularity: str = "minute"
    ) -> Dict[str, Any]:
...
class RedisRateLimitRepository(RateLimitRepository):
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            return RateLimitResult.fallback_result(request_key=key, error_details=str(e))
    ...
    async def _check_token_bucket(
        self, key: RateLimitKey, quota: RateLimitQuota, request_weight: int
    ) -> RateLimitResult:

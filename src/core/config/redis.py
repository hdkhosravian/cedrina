...
from pydantic import Field, PydanticUserError, SecretStr, ValidationInfo, field_validator
...
class RedisSettings(BaseSettings):
    ...
    RATE_LIMIT_STRATEGY: str = Field(
        default="fixed-window", pattern="^(fixed-window|sliding-window|token-bucket)$"
    )
    RATE_LIMIT_BLOCK_DURATION: int = Field(
        default=60, ge=1, description="Duration in seconds to block after rate limit exceeded"
    )
    ...
    def validate_rate_limit_format(cls, value: str) -> str:
        ...
        try:
            ...
            logger.error(f"Invalid rate limit format: {value}. Error: {e!s}")
            raise ValueError(f"Invalid rate limit format: {value}. " f"Must be 'count/period'.")

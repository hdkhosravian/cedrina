...
class MetricsCollector:
    ...
    def record_request_metric(
        ...
        self._metrics["application"]["requests"][key]["status_codes"][str(status_code)] = (
            self._metrics["application"]["requests"][key]["status_codes"].get(str(status_code), 0)
            + 1
        )

    def record_database_metric(self, operation: str, duration: float, success: bool) -> None:
    ...
    def record_cache_metric(self, operation: str, hit: bool) -> None:
        ...
        if operation not in self._metrics["cache"]["operations"]:
            self._metrics["cache"]["operations"][operation] = {"hits": 0, "misses": 0}
    ...
    def reset_metrics(self) -> None:
        ...
        self._metrics = {"system": {}, "application": {}, "database": {}, "cache": {}}
        self._start_time = datetime.now(timezone.utc)
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if metric_type == "database":
                    metrics_collector.record_database_metric(func.__name__, duration, True)
                elif metric_type == "cache":
                    metrics_collector.record_cache_metric(func.__name__, True)
                ...
                if metric_type == "database":
                    metrics_collector.record_database_metric(func.__name__, duration, False)
